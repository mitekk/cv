...
export const About: React.FC = () => {
  const paragraphs = [
    ...
    "I’m motivated by a simple idea: working together, learning from each other, and building solutions we can all feel proud of.",
  ];
      ...
      <div style={{ color: "#333332" }}>
        <div className="mb-4 flex flex-col gap-5 text-lg text-justify">
          ...
          {paragraphs.map((paragraph, index) => (
            <p
              key={index}
              className={`${index === paragraphs.length - 1 ? "mb-12" : ""}`}
            >
              {paragraph}
            </p>
          ))}
